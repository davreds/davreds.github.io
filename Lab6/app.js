...
app.get('/weather', function (req, res){
    ...
    if( !req.query.search) {
        return res.send({
            error: "Tienes que ingresar una  ciudad"
        })
    }
    weather.getLocation(req.query.search, function(error, response){
        ...
        weather.getWeather(coordinates.lat, coordinates.long, function(error, response){
            if(error) {
                return res.send({
                    error: error
                })
            ...
            res.send({
                ...
                weather: response.sum + " Actualmente estamos a " + response.temp + "°C. Hay " + response.prob + "% probabilidad de lluvia."
            })
        })
    })
})
